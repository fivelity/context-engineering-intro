export const spotlight = (node, options) => {
    if (options?.radius) {
    ...
    node.classList.add('relative', 'isolate', options?.radius && '[--spotlight-radius:var(--default-spotlight-radius)]', options?.borderWidth && '[--spotlight-border-width:var(--default-spotlight-border-width)]', options?.borderColorStops &&
        '[--spotlight-border-color-stops:var(--default-spotlight-border-color-stops)]', options?.surfaceColorStops &&
        '[--spotlight-surface-color-stops:var(--default-spotlight-surface-color-stops)]', options?.hover?.radius && 'hover:[--spotlight-radius:var(--hover-spotlight-radius)]', options?.hover?.borderWidth &&
        'hover:[--spotlight-border-width:var(--hover-spotlight-border-width)]', options?.hover?.borderColorStops &&
        'hover:[--spotlight-border-color-stops:var(--hover-spotlight-border-color-stops)]', options?.hover?.surfaceColorStops &&
    ...
    'before:absolute', 'before:inset-0', 'before:z-[-1]', 'before:[border:var(--spotlight-border-width)_solid_transparent]', 'before:[background:fixed_padding-box_radial-gradient(var(--spotlight-radius)_at_var(--x,0px)_var(--y,0px),var(--spotlight-surface-color-stops)),fixed_border-box_radial-gradient(var(--spotlight-radius)_at_var(--x,0px)_var(--y,0px),var(--spotlight-border-color-stops))]');
    return {
        destroy() {
            //
        },
